 ...
 * - **Obsidian shortcuts (optional)**: On macOS, runs matching Obsidian commands via `app.commands` so the PTY does not receive them.
 */
export class CustomKeyEventHandlerAddon implements ITerminalAddon {
  ...
  public constructor(
    ...
    protected readonly tryDispatchObsidianShortcut: (
      event: KeyboardEvent,
    ) => boolean = () => false,
  ) {}
  ...
  public activate(terminal: Terminal): void {
    const handler = (event: KeyboardEvent): boolean => {
      ...
      if (this.tryDispatchObsidianShortcut(event)) {
        return false;
      }
